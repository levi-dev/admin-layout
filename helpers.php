...
if (!function_exists('theme')) {
     ...
     *
     * @return \OctopusOsc\AdminLayout\Core\Adapters\Theme|\Illuminate\Contracts\Foundation\Application|mixed
     */
    function theme()
    {
        return app(\OctopusOsc\AdminLayout\Core\Adapters\Theme::class);
    }
}
...
if (!function_exists('util')) {
     ...
     *
     * @return \OctopusOsc\AdminLayout\Core\Adapters\Util|\Illuminate\Contracts\Foundation\Application|mixed
     */
    function util()
    {
        return app(\OctopusOsc\AdminLayout\Core\Adapters\Util::class);
    }
}
...
if (!function_exists('bootstrap')) {
    ...
    function bootstrap()
    {
        $bootstrap = "\OctopusOsc\AdminLayout\Core\Bootstraps\Bootstrap";
...
if (!function_exists('assetCustom')) {
    ...
    function assetCustom($path)
    {
        ...
        if (isRTL()) {
            return asset(theme()->getTheme().'/'.dirname($path).'/'.basename($path, '.css').'.rtl.css');
        }
        ...
        if (theme()->isDarkModeEnabled() && theme()->getCurrentMode() !== 'light') {
            ...
            if (file_exists(public_path(theme()->getTheme().'/'.$darkPath))) {
                return asset(theme()->getTheme().'/'.$darkPath);
            }
        }

        // Include default css file
        return asset('assets/' . theme()->getTheme().'/'.$path);
    }
}
